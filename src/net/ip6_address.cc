...
{
    return (addr32[0] == 0 && addr32[1] == 0 && addr32[2] == 0 && addr32[3] == 0);
}

bool Ip6Address::IsLoopback() const
{
    return (addr32[0] == 0 && addr32[1] == 0 && addr32[2] == 0 && addr32[3] == HostSwap32(1));
}

bool Ip6Address::IsLinkLocal() const
{
    return (addr8[0] == 0xfe) && ((addr8[1] & 0xc0) == 0x80);
}
...
{
    return (addr32[0] == HostSwap32(0xff020000) && addr32[1] == 0 &&
            addr32[2] == 0 && addr32[3] == HostSwap32(0x01));
}

bool Ip6Address::IsLinkLocalAllRoutersMulticast() const
{
    return (addr32[0] == HostSwap32(0xff020000) && addr32[1] == 0 &&
            addr32[2] == 0 && addr32[3] == HostSwap32(0x02));
}
...
{
    return (addr32[0] == HostSwap32(0xff030000) && addr32[1] == 0 &&
            addr32[2] == 0 && addr32[3] == HostSwap32(0x01));
}
...
{
    return memcmp(addr8, other.addr8, sizeof(addr8)) == 0;
}
...
ThreadError Ip6Address::FromString(const char *buf)
{
    ...
    uint8_t *dst = reinterpret_cast<uint8_t *>(addr8);
    uint8_t *endp = reinterpret_cast<uint8_t *>(addr8 + 15);
    uint8_t *colonp = NULL;
    uint16_t val = 0;
    uint8_t count = 0;
    bool first = true;
...
const char *Ip6Address::ToString(char *buf, uint16_t size) const
{
    snprintf(buf, size, "%x:%x:%x:%x:%x:%x:%x:%x",
             HostSwap16(addr16[0]), HostSwap16(addr16[1]),
             HostSwap16(addr16[2]), HostSwap16(addr16[3]),
             HostSwap16(addr16[4]), HostSwap16(addr16[5]),
             HostSwap16(addr16[6]), HostSwap16(addr16[7]));
    return buf;
}
