...
ThreadError Dhcp6Server::Start(const Ip6Address *address, Dhcp6ServerDelegate *delegate) {
  ...
  sockaddr.sin6_port = HostSwap16(kUdpServerPort);
  SuccessOrExit(error = socket_.Bind(&sockaddr));

  // netif_->SubscribeAllDhcpServersMulticast();
...
void Dhcp6Server::RecvFrom(void *context, Message *message, const Ip6MessageInfo *message_info) {
  Dhcp6Server *obj = reinterpret_cast<Dhcp6Server*>(context);
  obj->RecvFrom(message, message_info);
}

void Dhcp6Server::RecvFrom(Message *message, const Ip6MessageInfo *message_info) {
  Dhcp6Header header;
...
void Dhcp6Server::ProcessLeaseQuery(Message *message, const Ip6Address *source, const uint8_t *transaction_id) {
  ...
  if (delegate_->HandleLeaseQuery(&option.ia_address.address, &address, &last_transaction_time) == kThreadError_None) {
...
ThreadError Dhcp6Server::ProcessIaAddr(Message *message, uint16_t offset) {
  ...
  SuccessOrExit(error = delegate_->HandleReleaseAddress(&option.address));
...
ThreadError Dhcp6Server::AppendIaNa(Message *message, const ClientIdentifier *client_identifier) {
  ...
  if (delegate_->HandleGetAddress(client_identifier, &ia_address) == kThreadError_None) {
